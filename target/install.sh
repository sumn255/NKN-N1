...
if [ ! -f /boot/nosave.txt ];then
    echo "Backup docker directory"
    rm -f /root/dockerdir.tar
    rm -f /root/config.json
    mount /dev/mmcblk1p2 /mnt
    if [ -d /mnt/mnt/sda1/docker ];then
        cd /mnt/mnt/sda1/docker 
        tar -cf /root/dockerdir.tar ./swarm 
        cp ./volumes/nkn-mining_bin/_data/config.json /root/config.json
        cp ./volumes/nkn-mining_bin/_data/wallet.dat /root/wallet_`date +%Y-%m-%d-%H:%M:%S`.dat
        cd /root
    fi
